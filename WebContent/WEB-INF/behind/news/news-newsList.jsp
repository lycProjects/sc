...
<body>
<form action="news-new/delete" id="delete_form">
...
<div class="page-container">
	...
	<a class="btn btn-primary radius" onclick="picture_add('添加新闻','news-newsList-addnew')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加新闻</a></span> <span class="r"></span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			    <c:forEach items="${newsNewList }" var="newsNew">
					<tr class="text-c">
					 <td><input name="delete_form_ids" type="checkbox" value="${newsNew.newsnewId }"></td>
					 <c:forEach items="${newsCategoryList }" var="newsCategoryListItem">
					 	<c:if test="${newsCategoryListItem.newscateId==newsNew.newscateId }">
					 		<td>${newsCategoryListItem.newscateTitle }</td>
					 	</c:if>
					 </c:forEach>
					 <td>${newsNew.newsnewTitle }</td>
					 <td>${newsNew.newsnewBrief }</td>
					 <td>${newsNew.newsnewAuthorname }</td>
					 <td>${newsNew.newsnewPubtime }</td>
					 <td class="td-manage">
					 	<a style="text-decoration:none" onClick="picture_add('新闻 &nbsp;>&nbsp;《${newsNew.newsnewTitle }》','${pageContext.request.contextPath}${newsNew.newsnewHtmluri }')" href="javascript:;" title="查看"><i class="Hui-iconfont">&#xe6de;</i></a> 
					 	<a style="text-decoration:none" class="ml-5" onClick="picture_edit('编辑新闻','${pageContext.request.contextPath}/news/news-newsList-editnew','${newsNew.newsnewId }')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
					 </td>
					</tr>
				</c:forEach>
			</tbody>
...
function postDelete(index){
	...
	if(confirm('真的要删除吗？')){
		if(index<0){//批量删除
			...
			document.getElementById('newsNewIds').value=idsString.substring(1);
		}else{//单个删除
			document.getElementById('newsNewIds').value=index;
		}
		//alert(document.getElementById('newsNewIds').value);
		delete_form.submit();
	}
}
