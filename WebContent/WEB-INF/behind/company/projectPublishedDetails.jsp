...
<!-- 		<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 项目相关信息 <span class="c-gray en">&gt;</span> 项目市场
		...
		<div class="projectTopDiv">
			...
			<div class="projectTopDiv_name">
				<p>名称：${issusdProject.ipProjectname } <c:if test="${issusdProject.enProjectstate==2 && canDoAction==true }"><a class="btn btn-primary radius btn-danger" style="position: absolute;top:20px;right:40px;" onclick="endProject()" href="javascript:;">项目结项</a></c:if></p>
				...
				<table class="table table-border table-bordered table-hover table-bg table-sort">
					<thead>
						<tr class="text-c">
							...
							<c:if test="${issusdProject.enProjectstate==1 && canDoAction==true }">
								<th width="20">操作</th>
							</c:if>
						</tr>
					</thead>
					...
					<c:forEach items="${applications }" var="application">
						<tr class="text-c">
							<td><input type="checkbox" value="${application.apId }" name="ipId"></td>
							...
							<c:if test="${issusdProject.enProjectstate==1 && canDoAction==true }">
								<c:if test="${application.apState==1 }">
									<td class="td-manage" style="align-content: center;">
										<a style="text-decoration:none" href="javascript:;" onClick="disagreeApply(${application.apId})" title="拒绝"><button class="btn btn-mini btn-danger" type="button">拒绝</button></a>&nbsp;&nbsp;
										<a style="text-decoration:none" href="javascript:;" onClick="agreeApply(${application.apId})" title="同意"><button class="btn btn-mini btn-primary" type="submit">同意</button></a>
									</td>
								</c:if>
								<c:if test="${application.apState!=1 }">
									<td class="td-manage" style="align-content: center;">
										<a style="text-decoration:none" href="javascript:;" title="不可操作"><button class="btn btn-mini btn-danger" type="button">不可操作</button></a>
									</td>
								</c:if>
							</c:if>
						</tr>
		...
		function endProject() {
			layer.confirm('确定对该项目结项吗？', function(index) {
				//$(obj).remove();
				$.post("${pageContext.request.contextPath}/company/projectPublished/details/${issusdProject.ipId }/endProject",
			...
			function agreeApply(id) {
				layer.confirm('确认同意该申请吗？', function(index) {
					//$(obj).remove();
					$.post("${pageContext.request.contextPath}/company/projectPublished/details/${issusdProject.ipId }/agree/"+id,
				    {},function(data,status){
						if(data=="success"){
							layer.msg('已同意!', {
								icon: 6,
			...
			function disagreeApply(id) {
				layer.confirm('确认拒绝该申请吗？', function(index) {
					//$(obj).remove();
					$.post("${pageContext.request.contextPath}/company/projectPublished/details/${issusdProject.ipId }/disagree/"+id,
				    {},function(data,status){
						if(data=="success"){
							layer.msg('已拒绝!', {
								icon: 6,
