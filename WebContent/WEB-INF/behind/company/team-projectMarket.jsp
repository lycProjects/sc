		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			    <c:forEach items="${issusdProjectList }" var="issusdProject">
					...
					<td>${issusdProject.ipProjectname}</td>
					<td>${issusdProject.enStatename}</td>
					<td>${fn:substring(issusdProject.ipBriefintroduction,0,50)}...</td>
					<td>${issusdProject.enIssusdtime}</td>
					<td><button class="btn btn-mini btn-danger" type="button" onClick="picture_add('查看项目&nbsp;>&nbsp;《${issusdProject.ipProjectname }》','${pageContext.request.contextPath}/team/IssusdProject/${issusdProject.ipId}/details')" >查看项目详情</button></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
