...
<body>
<form action="${pageContext.request.contextPath}/company/projectPublished/delete" id="delete_form" method="post">
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			    <c:forEach items="${issusdProjectList }" var="issusdProject">
			     <tr>
			    	<td><input name="ip_id" type="checkbox" value="${issusdProject.ipId }"></td>
					<td><a href="javascript:;" onclick="picture_add('查看项目 > ${issusdProject.ipProjectname}','${pageContext.request.contextPath}/company/projectPublished/details/${issusdProject.ipId}?canDoAction=true')">${issusdProject.ipProjectname}</a></td>
					<td>${issusdProject.enStatename}</td>
					<td>${fn:substring(issusdProject.ipBriefintroduction,0,50)}...</td>
					<td>${issusdProject.enIssusdtime}</td>
					<td class="td-manage">
					 	<a style="text-decoration:none" onClick="picture_add('查看项目&nbsp;>&nbsp;《${issusdProject.ipProjectname }》','${pageContext.request.contextPath}/company/projectPublished/details/${issusdProject.ipId}?canDoAction=true')" href="javascript:;" title="详情"><i class="Hui-iconfont">&#xe6de;</i></a> 
					 	<a style="text-decoration:none" class="ml-5" onClick="picture_edit('编辑新闻','${pageContext.request.contextPath}/company/projectPublished/edit/${issusdProject.ipId}','')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
					 </td>
				  </tr>
				</c:forEach>
			</tbody>
...
function deleteForm(){
	layer.confirm('确定删除所有勾选的项目吗？', function(index) {
